= Izaak Alpert (bastardized from Jduff/Ryan Bates) Dot Files
...
== Installation
  ...
  git clone git://github.com/karlhungus/dotfiles ~/.dotfiles
  cd ~/.dotfiles
  rake install
...
Things I've done but haven't automated:

 - CommandT
 `rake make in vim/bundle/command-t dir
 - YouCompleteMe (might need to check the pages
  ```
    sudo apt-get install cmake #(for you complete me)
    ...
    # For specific support (javascript/gocode look at the YouCompleteMe page here:
